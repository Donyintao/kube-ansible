...
###################################################################################################################################
#####cni:         wget https://github.com/containernetworking/plugins/releases/download/v0.9.0/cni-plugins-linux-amd64-v0.9.0.tgz
#####etcd:        wget https://github.com/etcd-io/etcd/releases/download/v3.4.14/etcd-v3.4.14-linux-amd64.tar.gz
...
# NODE
KUBE_NODE_1_IP="192.168.23.133"
KUBE_NODE_2_IP="192.168.23.134"
KUBE_NODE_3_IP="192.168.23.135"
...
ETCD_CONF_PATH="${ETCD_HOME_PATH}/conf"
ETCD_DATA_PATH="${ETCD_HOME_PATH}/data"
ETCD_LOGS_PATH="${ETCD_HOME_PATH}/logs"
# ETCD 集群通信地址端口
...
# KUBELET存储目录
KUBE_DATA_PATH="${DATA_PATH}/kubelet/data"
# 集群(POD)网段
...
# Master VIP域名
KUBE_CLUSTER_VIP_DOMAIN="kube-master.${KUBE_DNS_DOMAIN}"
...
if [ ! -d "kubernetes" ]; then
    ...
    tar fx etcd-${ETCD_APP_VERSION}-linux-amd64.tar.gz
fi
# 下载k8s文件(需要科学上网)
if [ ! -d "kubernetes" ]; then
    wget -q https://storage.googleapis.com/kubernetes-release/release/${KUBE_APP_VERSION}/kubernetes-server-linux-amd64.tar.gz
    tar fx kubernetes-server-linux-amd64.tar.gz
fi
# 下载cni文件
if [ ! -d "cni" ]; then
    wget -q https://github.com/containernetworking/plugins/releases/download/${KUBE_CNI_VERISON}/cni-plugins-linux-amd64-${KUBE_CNI_VERISON}.tgz
    mkdir -p cni && tar fx cni-plugins-linux-amd64-${KUBE_CNI_VERISON}.tgz -C cni
fi
###################################################################################################################################
...
# 创建ETCD SERVER CA证书签名请求
cat << EOF | tee ${CFSSL_PATH}/etcd-server-ca-csr.json 
{
  "CN": "etcd",
  "key": {
    "algo": "rsa",
...
EOF
# 创建ETCD CLIENT CA证书签名请求
cat << EOF | tee ${CFSSL_PATH}/etcd-client-ca-csr.json 
{
  "CN": "etcd",
  "key": {
    "algo": "rsa",
...
cfssl gencert -initca ${CFSSL_PATH}/etcd-server-ca-csr.json | cfssljson -bare ${CFSSL_PATH}/etcd-server-ca
# 生成ETCD SERVER证书和私钥
cfssl gencert \
    -ca=${CFSSL_PATH}/etcd-server-ca.pem \
    -ca-key=${CFSSL_PATH}/etcd-server-ca-key.pem \
    -config=${CFSSL_PATH}/ca-config.json \
    -profile=kubernetes ${CFSSL_PATH}/etcd-server-csr.json | cfssljson -bare ${CFSSL_PATH}/etcd-server

# 生成ETCD CLIENT CA证书和私钥
cfssl gencert -initca ${CFSSL_PATH}/etcd-client-ca-csr.json | cfssljson -bare ${CFSSL_PATH}/etcd-client-ca
# 生成ETCD CLIENT证书和私钥
cfssl gencert \
    -ca=${CFSSL_PATH}/etcd-client-ca.pem \
    -ca-key=${CFSSL_PATH}/etcd-client-ca-key.pem \
    -config=${CFSSL_PATH}/ca-config.json \
...
# 创建ETCD Palybook目录
mkdir -p ${PLAYBOOK_PATH}/roles/etcd/{files/{bin,conf,ssl},handlers,tasks,templates}
# 拷贝bin文件到Playbook目录
/bin/cp -rf ${TEMP_PATH}/etcd-${ETCD_APP_VERSION}-linux-amd64/{etcd,etcdctl} ${PLAYBOOK_PATH}/roles/etcd/files/bin
...
#[Member]
ETCD_DATA_PATH="${ETCD_DATA_PATH}/{{ ansible_hostname }}.etcd"
...
#[Security client]
ETCD_CERT_FILE="${ETCD_CONF_PATH}/ssl/etcd-client.pem"
ETCD_KEY_FILE="${ETCD_CONF_PATH}/ssl/etcd-client-key.pem"
ETCD_TRUSTED_CA_FILE="${ETCD_CONF_PATH}/ssl/etcd-client-ca.pem"
ETCD_CLIENT_CERT_AUTH="true"
ETCD_AUTO_TLS="true"

#[Security server]
ETCD_PEER_CERT_FILE="${ETCD_CONF_PATH}/ssl/etcd-server.pem"
ETCD_PEER_KEY_FILE="${ETCD_CONF_PATH}/ssl/etcd-server-key.pem"
ETCD_PEER_TRUSTED_CA_FILE="${ETCD_CONF_PATH}/ssl/etcd-server-ca.pem"
ETCD_PEER_CLIENT_CERT_AUTH="true"
ETCD_PEER_AUTO_TLS="true"

#[Logging]
ETCD_LOGGER="zap"
ETCD_LOG_PACKAGE_LEVELS="etcdmain=CRITICAL,etcdserver=DEBUG"
ETCD_LOG_OUTPUTS="stderr,${ETCD_LOGS_PATH}/etcd.log"
EOF
# 创建ETCD启动文件
...
ExecStart=/bin/bash -c "GOMAXPROCS=$(nproc) ${ETCD_HOME_PATH}/bin/etcd"
Restart=on-failure
LimitNOFILE=65536
...
- name: mkdir etcd conf directory
  file:
    ...
    state: directory
  with_items:
    - { path: "${ETCD_DATA_PATH}", mode: "0700"}
    - { path: "${ETCD_LOGS_PATH}", mode: "0755"}
    - { path: "${ETCD_CONF_PATH}", mode: "0755"}
...
cat << EOF | tee ${PLAYBOOK_PATH}/roles/etcd/handlers/main.yml
...
# 创建docker Palybook目录
mkdir -p ${PLAYBOOK_PATH}/roles/docker/{handlers,tasks,templates}
# 创建docker Playbook文件
...
cat << EOF | tee ${PLAYBOOK_PATH}/roles/docker/handlers/main.yml
...
[Service]
WorkingDirectory=${KUBE_DATA_PATH}
EnvironmentFile=-${KUBE_LINK_PATH}/etc/kube-config
